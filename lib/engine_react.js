...
function babelTransform (pattern) {
  return Babel.transform(moduleCodeString(pattern), {
...
function customRequire (id) {
  ...
  if (registeredPattern) {
    console.log('pattern already registered: ', registeredPattern.name);
    const transpiledModule = babelTransform(registeredPattern);
    ((require) => {
      /* eslint-disable no-eval */
      registeredPattern.module = eval(transpiledModule.code);
    })(customRequire);
    return registeredPattern.module;
  } else if (id !== 'react') {
    console.log('pattern not registered and not react');
    return _require(id);
  } else {
    // console.log('pattern is react');
    return _require(id);
  }
...
var engine_react = {
  ...
  renderPattern(pattern, data, partials) {
    console.log('RENDER: ', pattern.name);
    try {
      ...
      ((require) => {
        /* eslint-disable no-eval */
        pattern.module = eval(transpiledModule.code);
      })(customRequire);
  ...
  registerPartial(pattern) {
    // // the all-important Babel transform
    // const compiledModule = babelTransform(pattern);
