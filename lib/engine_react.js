...
function generateServerScript(pattern) {
    ...
    webpack(
      {
        ...
        output: {
          ...
          library: 'blob',
          libraryTarget: 'umd',
          path: getAbsolutePatternOutputDir(pattern),
        },
...
function generateClientScript(pattern) {
  ...
  return new Promise((resolve, reject) => {
    ...
    compiler.run((err, stats) => {
      ...
      if (err) {
        ...
        reject('something went wrong in generateClientScript():', err);
      }
...
const engine_react = {
  ...
  renderPattern(pattern, data, partials) {
    ...
    return Promise.resolve()
      .then(() => {
        return generateServerScript(pattern);
      })
      .then(() => {
        // write entry .js file
...
`;
        fs.writeFileSync(
          path.join(getAbsolutePatternOutputDir(pattern), 'clientSideEntry.js'),
          entryFileContents,
          { encoding: 'utf8' }
        );
      })
      .then(() => {
        return generateClientScript(pattern);
      })
      .then(clientSideScript => {
