...
const clientSideCompiler = () => {
  const tempFS = new MemoryFS();
  let fileContent = "still not working";
  ...
  compiler.run((err, stats) => {
    ...
    console.log(stats.hasWarnings() ? "There were warnings." : "No warnings.");
    if (stats.hasErrors()) {
      console.log('There were errors.');
      jsonStats.errors.forEach((err2) => {
        console.log('webpack error:', err2);
      });
    }

    // read contents of fake file and display
    fileContent = tempFS.readFileSync('/foo.js', 'utf8');
    console.log('hooray!', fileContent);
  });
...
var engine_react = {
  ...
  renderPattern(pattern, data, partials) {
    try {
      const runtimeCode = clientSideCompiler();
